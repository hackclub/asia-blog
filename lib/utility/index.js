...
const doesPropertyExists = _.curry((object, property) => _.pipe(_.keys(), _.includes(property))(object))
...
const getSectionData = sectioname => {
  ...
  const posterurl = getPosterUrl(sectioname, file)
    return push(acc)({title, author, para, posterurl}) // push(acc)([ { title, author, para, posterurl }])
  })
...
const getPosterUrl = _.curry((section, filename, forSlug = false) => {
  ...
  if (posterimgurlAlreadyExists)
    return _.prop('posterimgurl')(blogdata)
